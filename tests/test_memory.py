...
def run_tests():
    ...
    try:
        ...
        result_id = memory.add_learning(
            ...
            content="Row-level security combined with schema isolation is effective for multi-tenant apps",
        ...
        test("Learning content stored", "Row-level security" in data["learnings"][0]["content"])
        ...
        test("Query returns correct content", "Row-level security" in arch_learnings[0]["content"])
        ...
        corr_id = memory.add_correction(
            corrector="test",
            original_claim="Row-level security is always sufficient",
            correction="Row-level security is sufficient for most SaaS but schema-per-tenant adds defense-in-depth for high-compliance apps",
            category="architecture",
        )
