...
if ($needsConfig) {
    ...
    switch ($choice) {
        "1" {
            $apiKey = Read-MaskedInput "Enter your xAI API key (from https://console.x.ai/)"
            $provider = "grok"
            $model = "grok-4-1-fast-reasoning"
        }
        "2" {
            $apiKey = Read-MaskedInput "Enter your Gemini API key (from https://aistudio.google.com/apikey)"
            $provider = "gemini"
            $model = "gemini-2.0-flash"
        }
        "3" {
            $apiKey = Read-MaskedInput "Enter your OpenAI API key (from https://platform.openai.com/api-keys)"
            $provider = "openai"
            $model = "gpt-4o"
        }
        "4" {
            $apiKey = Read-MaskedInput "Enter your DeepSeek API key (from https://platform.deepseek.com/)"
            $provider = "deepseek"
            $model = "deepseek-chat"
        }
        default {
