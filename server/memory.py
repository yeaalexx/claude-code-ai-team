...
def _detect_category(text: str) -> str:
    ...
    if any(w in text_lower for w in ["compliance", "regulation", "audit", "certification", "governance"]):
        return "compliance"
    if any(w in text_lower for w in ["integration", "contract", "cross-service", "header propagation", "x-tenant"]):
        return "integration"
    if any(w in text_lower for w in ["domain", "business logic", "workflow", "data model", "entity"]):
        return "domain"
